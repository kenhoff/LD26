...
function OnGUI () {
	...
	if (LookAtSnape == false) {
		...
		h = 25;
		w = 100;
		if (GUI.Button (Rect((Screen.width / 2) - w/2, (Screen.height / 2) - h/2 + 150 + h, w, h), "How to play")) {
			// Application.LoadLevel("scene1");
			LookAtSnape = true;
		}
	}
	if (LookAtSnape == true) {
		h = 25;
		w = 100;
		if (GUI.Button (Rect((Screen.width / 2) - w/2, (Screen.height / 2) - h/2 + 150 + h, w, h), "back")) {
			// Application.LoadLevel("scene1");
